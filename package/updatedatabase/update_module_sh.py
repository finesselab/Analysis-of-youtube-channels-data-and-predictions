...
#check_if_video_exists('C-fEfJL3Wp4')



def update_row(video_id, view_count, like_count):
    query = '''
            UPDATE broda_shaggi3
            SET 
                view_count = ?,
                like_count = ?
            WHERE video_id = ?
...
def update_db(df):
    temp_df = pd.DataFrame(columns = ['video_id', 'date', 'time', 'title', 'cast1', 'cast2', 'cast3', 'cast4',
       'cast5', 'cast6', 'cast7', 'view_count', 'like_count', 'dislike_count', 'comment_count', 'video_duration'])
...
def insert_new_data_to_table(video_id, date, time, title, cast1, cast2, cast3, cast4,
            cast5, cast6, cast7, view_count, like_count, dislike_count, comment_count, video_duration):
    query = '''
            INSERT INTO broda_shaggi3 (video_id, upload_date, upload_time, title, cast1, cast2, cast3, cast4,
            cast5, cast6, cast7, view_count, like_count, dislike_count, comment_count, duration) 
            VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    '''
...
def append_new_data_to_db(dff):
    for i, row in dff.iterrows():
        insert_new_data_to_table(row['video_id'], row['date'], row['time'], row['title'],
                         row['cast1'],row['cast2'], row['cast3'], row['cast4'], row['cast5'], row['cast6'], row['cast7'],
